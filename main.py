...
from ai_services import analyze_file, analyze_message, analyze_site
...
SCAM_BUTTONS = ["Проверка SMS (AI)", "Проверка SMS", "🛡 Проверка SMS"]
PASSWORD_BUTTONS = ["Пароли", "🔐 Пароли"]
DOMAIN_BUTTONS = ["Домены", "🖥 Домены"]
IP_BUTTONS = ["IP", "📡 IP"]
SITE_BUTTONS = ["Проверка сайта (AI)", "Проверка сайта", "🧭 Проверка сайта"]
FILE_BUTTONS = ["Проверка файла (AI)", "Проверка файла", "Файлы", "📁 Файлы"]
EMAIL_BUTTONS = ["Email", "📧 Email"]
CRYPTO_BUTTONS = ["Шифратор", "🔐 Шифратор"]
ADDITIONAL_MENU_BUTTONS = ["Доп. функции", "🧰 Доп. функции"]
MAIN_MENU_BUTTONS = ["Главное меню", "⬅️ Главное меню"]
SCAM_CHECK_TEXT = '''<tg-emoji emoji-id="5219805369806629055">🛡</tg-emoji> Проверка на мошенничество (фишинг)
...
@dp.message(F.text.in_(ADDITIONAL_MENU_BUTTONS))
async def show_additional_menu(message: Message, state: FSMContext):
    await state.clear()
    await message.answer(
...
async def show_main_menu(message: Message, state: FSMContext):
    ...
    await send_main_photo(
        message,
        get_main_menu_text(message.from_user),
        menu,
    )
...
async def site_menu(message: Message, state: FSMContext):
    ...
    await send_section_photo(
        message,
        SITE_IMAGE,
        '''<b><tg-emoji emoji-id="6019098624678435283">🧭</tg-emoji> Проверка сайта (AI)</b>
...
async def files_menu(message: Message, state: FSMContext):
    ...
    await send_section_photo(
        message,
        FILE_IMAGE,
        '''<b><tg-emoji emoji-id="6021859047404214713">📁</tg-emoji> Проверка файла (AI)</b>

<i>Выберите проверку файла.</i>

<b>Вирусы</b> проверит Kaspersky, а ИИ объяснит результат.
...
async def files_menu_callback(callback: CallbackQuery, state: FSMContext):
    ...
    await edit_section_photo(
        callback.message,
        FILE_IMAGE,
        '''<b><tg-emoji emoji-id="6021375494216226506">📁</tg-emoji> Проверка файла (AI)</b>
...
<i>ИИ проанализирует ссылку, домен и признаки угрозы.</i>''',
        get_wait_input_keyboard("site_menu"),
    )
    await save_wait_message(state, callback.message, message_id)
    await callback.answer()
...
def check_kaspersky_domain(domain):
    api_url = (
        "https://opentip.kaspersky.com/api/v1/search/domain?"
        + urllib.parse.urlencode({"request": domain})
    )
    request = urllib.request.Request(
        api_url,
        headers={
...
def check_site_virus_info(text):
    ...
    try:
        domain_is_ip = ipaddress.ip_address(domain) is not None
    except Exception:
        domain_is_ip = False

    if parsed_url.scheme not in ["http", "https"] or not (is_domain(domain) or domain_is_ip):
        answer = f'''<b><tg-emoji emoji-id="5276240711795107620">❌</tg-emoji> Это не похоже на сайт</b>

<b>Ввод:</b> <code>{url_html}</code>

<i>Отправьте домен или ссылку. Например: <code>example.com</code></i>'''
    ...
    kaspersky_zone = ""
    if KASPERSKY_OPENTIP_API_KEY:
        try:
            kaspersky_data = check_kaspersky_domain(domain)
            kaspersky_zone = kaspersky_data.get("Zone", "")
        except Exception:
            pass

    if matches or kaspersky_zone in ["Red", "Orange", "Yellow"]:
        reputation_status = "dangerous"
    elif kaspersky_zone == "Green":
    ...
    if google_checked:
        if matches:
            reputation_parts.append("Google Safe Browsing: найдены угрозы")
        else:
            reputation_parts.append("Google Safe Browsing: совпадений нет")
    if kaspersky_zone:
        reputation_parts.append("Kaspersky OpenTIP: " + kaspersky_zone)
    if not reputation_parts:
        reputation_parts.append("базы угроз недоступны")

    reputation_text = "; ".join(reputation_parts)
    return analyze_site(url, reputation_status, reputation_text)
...
def get_file_kaspersky_result(file_hash):
    url = (
        "https://opentip.kaspersky.com/api/v1/getresult/file?"
        + urllib.parse.urlencode({"request": file_hash})
    )
    request = urllib.request.Request(
        url,
        data=b"",
...
def check_file_virus_info(document, file_bytes):
    ...
    if KASPERSKY_OPENTIP_API_KEY:
        try:
            ...
            if error.code == 400:
                kaspersky_error = "файл не принят на анализ"
            elif error.code in [401, 403]:
                kaspersky_error = "ошибка API-ключа или лимита"
            else:
                kaspersky_error = "ошибка HTTP " + str(error.code)
        except Exception:
            kaspersky_error = "сервис не ответил"
    else:
        kaspersky_error = "API-ключ не настроен"

    if not isinstance(data, dict):
        data = {}

    first_file_info = data.get("FileGeneralInfo", {})
    if not isinstance(first_file_info, dict):
        first_file_info = {}

    first_zone = data.get("Zone", "")
    first_status = data.get("Status", "")
    file_hash = first_file_info.get("Sha256") or hashlib.sha256(file_bytes).hexdigest()

    if not kaspersky_error and (first_zone == "Grey" or first_status == "in progress"):
        for number in range(3):
            time.sleep(3)
            try:
                updated_data = get_file_kaspersky_result(file_hash)
            except Exception:
                break
            ...
            if data.get("Status") == "complete" or data.get("Zone") in ["Red", "Yellow", "Green"]:
                break
    ...
    if status == "in progress" or zone == "Grey" or not zone:
        reputation_status = "unknown"
    elif zone in ["Red", "Orange", "Yellow"]:
        reputation_status = "dangerous"
    elif zone == "Green":
        reputation_status = "safe"
    else:
    ...
    file_data = {
        ...
        "mime_type": document.mime_type or "не найдено",
        "zone": zone,
        "status": status,
        "file_status": file_status,
        "sha256": sha256,
        "md5": md5,
        "kaspersky_error": kaspersky_error,
    }
    return analyze_file(file_data, reputation_status)
...
async def check_site_virus_message(message: Message, state: FSMContext):
    await state.clear()
    answer, style = await asyncio.to_thread(check_site_virus_info, message.text or "")
    await send_section_photo(
        message,
        SITE_IMAGE,
        answer,
        get_site_result_keyboard(style),
    )
...
<i>Отправьте файл размером до 20 МБ.</i>'''
            ...
            try:
                ...
                answer, style = await asyncio.to_thread(check_file_virus_info, document, file_bytes)
                keyboard = get_check_again_keyboard("file_virus", "files_menu", style)
            except Exception:
